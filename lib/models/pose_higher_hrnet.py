...
class PoseHigherResolutionNet(nn.Module):
    ...
    #         #判断是否需要拼接输入特征图和上一级特征图：x为当前特征图，y是上一级输出
    ...
    #         final_outputs.append(y)
    #     # print(final_outputs[-1].shape)
    #     return final_outputs,stage2_high,stage3_high,stage4_high
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
